...
class PostConroller extends Controller
{
  ...
  {
    // 🟢 리스트를 보여주는 기능을 수행한다.
    // DB의 post 테이블의 레코드들을 읽어 온다. 
    // $posts = Post::all(); //🟠  Post::all();은 'posts' 테이블에 있는 모든 레코드를 가져오라는 의미   = select * from post; 이렇게 가져온다., 근데 이건 그냥 레코드 가져 오는 거다
  ...
  {
    return view('posts.register_form'); // 작성하는 폼 화면만 보여주면 된다.  
    // 여기서 store 메소드로 작성한 내용 날린다 ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
  }
  ...
  public function store(Request $request)
  {
    ...
    //  $request->merge() 메서드는 HTTP 요청의 데이터를 수정하거나 추가하는데 사용
  ...
  public function show(string $id)
  {
    ...
    /*  ❓❓❓❓❓❓  get() 메소드를 쓰면  왜 컬렉션을 반환하니 ❓❓❓❓❓❓❓❓
      Laravel의 Eloquent ORM에서 🔸get() 메소드🔸는 데이터베이스로부터 레코드를 조회하여 그 결과를 🔸컬렉션 형태🔸로 반환합니다.
